...
use Braunstetter\DataGridBundle\Contracts\GridRendererInterface;
...
class GridRenderer implements GridRendererInterface
{
    ...
    private GridRendererEngineInterface $engine;
    private array $blockNameHierarchyMap = [];
    private array $hierarchyLevelMap = [];
    private array $variableStack = [];

    public function __construct(GridRendererEngineInterface $engine)
    {
        $this->engine = $engine;
    }
    ...
    public function renderBlock(GridView $view, string $blockName, array $variables = []): string
    {
        ...
        if (!$resource) {
            throw new LogicException(sprintf('No block "%s" found while rendering the grid.', $blockName));
        }
